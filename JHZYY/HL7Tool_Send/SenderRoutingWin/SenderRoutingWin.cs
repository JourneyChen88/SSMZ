...
namespace SenderRoutingWin
{
    public partial class Form1 : Form
    {
        ...
        {
            FilePath = Configuration.getSetting("FilePath");
            this.toolStripStatusLabel1.Text = string.Format("当前数据文件路径:{0}", FilePath);
            this.toolStripStatusLabel2.Text = string.Format("当前接收端IP:{0}，端口:{1}", Configuration.getSetting("Address"), Configuration.getSetting("Port"));
        }
        ...
        private void btnReadData_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("没有选择数据文件，无法构建消息");
                return;
            }
            messageList = AnalysisXML.Analysis(FilePath);
            this.txtOutput.AppendText(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
            this.txtOutput.AppendText("读取消息数据完成");
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnBuildeMessage_Click(object sender, EventArgs e)
        {
