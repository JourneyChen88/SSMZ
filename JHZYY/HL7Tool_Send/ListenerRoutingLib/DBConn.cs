...
namespace ListenerRoutingLib
{
    public class DBConn
    {
        ...
        {
            var paiban = db.Queryable<OTypesetting>().Where(a => a.PatZhuYuanID == zhuyuanid).First();
            paiban.Ostate = Ostate;
            return db.Updateable<OTypesetting>(paiban).ExecuteCommand();
