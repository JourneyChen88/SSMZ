    ...
    public partial class ShoushuYYTJ : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                DataTable dt1 = bll.SelectQT(mzid);//气体查询
                DataTable dt2 = bll.selectYDY(mzid);//全麻
                DataTable dt3 = bll.selectJumaYao(mzid);//局麻
                DataTable dt4 = bll.getTSYY(mzid);// 特殊用药 
                DataTable dt5 = bll.GetsqyyUse(mzid);//术前
                DataTable dt6 = bll.selectSX1(mzid);//输血查询
                DataTable dt7 = bll.selectSY1(mzid);//输液查询
                ...
                foreach (DataRow dr in dt2.Rows)
                {
                    if (Convert.ToInt32(dr["Cxyy"]) == 1)
                    {
                        ...
                        dataGridView1.Rows.Add(i.ToString(), dr["ydyname"].ToString(), zongl.ToString(), dw);
        ...
        {
            DataTable dt1 = dal.GetMzjldBYtime(dtpStart.Value.Date.ToString("yyyy-MM-dd"));
