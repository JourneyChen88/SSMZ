...
namespace main
{
    public class ClassFTP
    {
        ...
        public ClassFTP(string ftpUser, string ftpPassWord)
        {
        ...
        //下载单个xml文件
        public void DownloadXMLfile(string filePath, string fileName)
        {
            FtpWebRequest reqFTP;

            try
            {
                FileStream outputStream = new FileStream(filePath + "/" + fileName, FileMode.Create);  //文件流，下载文件保存到客户端目录地址
                string ftpServerIP = "172.20.12.23:2121";
                string ftpUserID = "administrator";
                string ftpPassword = "admin";
                reqFTP = (FtpWebRequest)FtpWebRequest.Create(new Uri("ftp://" + ftpServerIP + "/" + fileName));  //创建FtpWebRequest对象，由Uri指定下载的文件
