...
namespace main
{
    public partial class BeforeVisit_JH : Form
    {
        bool xinjian = true;
        DataTable table1_copy;
        DataTable table1;
        adims_DAL.MkLisHelper hisoracl = new adims_DAL.MkLisHelper();
        ...
        string ZYNumber1;
        bool isRead = false;
        public BeforeVisit_JH(string ZYNumber)
        {
            ZYNumber1 = ZYNumber;
            InitializeComponent();
        }
        ...
        private void jcxjz()
        {
            string mc = textBoxzhuyuanhao.Text;
            DataTable cd = DAL.Getkcyshengshuqianfs(mc);
            if (cd.Rows.Count == 0)
            {
                DataTable dt = hisoracl.GetLIS_ReportMicroView1(mc, "B_HGB");
                if (dt.Rows.Count != 0)
                {

                    string B_HGB = dt.Rows[0]["TESTRESULT"].ToString();
                    dt = hisoracl.GetLIS_ReportMicroView1(mc, "B_HCT");
                    string B_HCT = dt.Rows[0]["TESTRESULT"].ToString();
                    textBoxHB.Text = B_HGB +"/"+ B_HCT;
                    //textBoxHB.Text = dt.Rows[0]["TESTRESULT"].ToString();
                }
                dt = hisoracl.GetLIS_ReportMicroView1(mc, "B_WBC");
                if (dt.Rows.Count != 0)
                {
                    textBoxwbc.Text = dt.Rows[0]["TESTRESULT"].ToString();
                }
                dt = hisoracl.GetLIS_ReportMicroView1(mc, "B_PLT");
                if (dt.Rows.Count != 0)
                {
                    textBoxplt.Text = dt.Rows[0]["TESTRESULT"].ToString();
                }
                dt = hisoracl.GetLIS_ReportMicroView1(mc, "K");
                if (dt.Rows.Count != 0)
                {
                    textBoxK.Text = dt.Rows[0]["TESTRESULT"].ToString();
                } dt = hisoracl.GetLIS_ReportMicroView1(mc, "Na");
                if (dt.Rows.Count != 0)
                {
                    textBoxNa.Text = dt.Rows[0]["TESTRESULT"].ToString();
                }
                dt = hisoracl.GetLIS_ReportMicroView1(mc, "Cl");
                if (dt.Rows.Count != 0)
                {
                    textBoxc1.Text = dt.Rows[0]["TESTRESULT"].ToString();
                } dt = hisoracl.GetLIS_ReportMicroView1(mc, "GLU");
                if (dt.Rows.Count != 0)
                {
                    textBoxGLU.Text = dt.Rows[0]["TESTRESULT"].ToString();
                }
                dt = hisoracl.GetLIS_ReportMicroView1(mc, "ALT");
                if (dt.Rows.Count != 0)
                {
                    textBoxSGPT.Text = dt.Rows[0]["TESTRESULT"].ToString();
                } dt = hisoracl.GetLIS_ReportMicroView1(mc, "BUN");
                if (dt.Rows.Count != 0)
                {
                    textBoxBUN.Text = dt.Rows[0]["TESTRESULT"].ToString();
                }
                dt = hisoracl.GetLIS_ReportMicroView1(mc, "CREA-P");
                if (dt.Rows.Count != 0)
                {
                    textBoxCR.Text = dt.Rows[0]["TESTRESULT"].ToString();
                }
                dt = hisoracl.GetLIS_ReportMicroView1(mc, "XN_PT");
                if (dt.Rows.Count != 0)
                {
                    textBoxPT.Text = dt.Rows[0]["TESTRESULT"].ToString();
                }
                dt = hisoracl.GetLIS_ReportMicroView1(mc, "APTT");
                if (dt.Rows.Count != 0)
                {
                    textBoxAPTT.Text = dt.Rows[0]["TESTRESULT"].ToString();
                } 
                dt = hisoracl.GetLIS_ReportMicroView1(mc, "pO2");
                if (dt.Rows.Count != 0)
                {
                    textBoxPAO2.Text = dt.Rows[0]["TESTRESULT"].ToString();
                }
            }
        }
        private void mzffbd()
        ...
        private void btnDZBL_Click(object sender, EventArgs e)
        {
            ...
            brsyh = DAL.brsyh(ZYNumber1);//获取病人就诊卡号
        ...
        {
            string zhuyuanhao = textBoxzhuyuanhao.Text;
            string url = "http://172.1.1.122/IMCISClient/Interface/ClinicExamList?UserID=his&UserPwd=123456&InPatientNo=" + zhuyuanhao + "";
            System.Diagnostics.Process.Start(url);
        }

        private void buttonjybl_Click(object sender, EventArgs e)
        {
            DataTable zhi=DAL.brsyh(ZYNumber1);
        ...
        {
            string zhuyuanhao = textBoxzhuyuanhao.Text;
            string url = "http://172.1.1.73/HisReportList.aspx?ClinicCode=" + zhuyuanhao + "";
            System.Diagnostics.Process.Start(url);
        }
