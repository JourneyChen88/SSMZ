...
namespace main.权限管理
{
    public partial class szsjGuanli : Form
    {
        ...
        {
            DataTable dt = dal.GetAdims_szsjListByType(cmbSJType.Text.Trim());
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (txtSJname.Text != "" && cmbSJType.Text != "")
            {
                ...
                DataTable dt = dal.GetAdims_szsjListByType(name1);
                if (dt.Rows.Count == 0)
                {
                    i = dal.InsertAdims_szsjList(name1,suoxie1, type1);
                    BinddgvShijian();
                    if (i == 0)
                    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (dgvShijian.SelectedRows.Count > 0)
            {
                ...
                type1 = cmbSJType.Text.Trim();
                suoxie1 = tbSJSuoxie.Text.Trim();
                DataTable dt = dal.GetAdims_szsjListByType(name1, type1);
                if (dt.Rows.Count == 0)
                {
                    flag = dal.UpdateAdims_szsjList(name1, suoxie1, type1, Convert.ToInt32(dgvShijian.SelectedRows[0].Cells[0].Value));
                    BinddgvShijian();
                    if (flag == 0)
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            {
                int flag = 0;
                flag = dal.deleteAdims_szsjList(Convert.ToInt32(dgvShijian.SelectedRows[0].Cells[0].Value));
                BinddgvShijian();
                if (flag == 0)
                {
