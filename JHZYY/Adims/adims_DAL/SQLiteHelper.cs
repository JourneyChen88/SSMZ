...
namespace adims_DAL
{
    public class SQLiteHelper
    {
        ...
        {
            string query = "select mzjldid,CreateTime,NIBPS,NIBPD,NIBPM,RRC,HR,Pulse,SpO2,ETCO2,TEMP,ARTS from Adims_MonitorRecord "
                + " where mzjldid='" + mzjldid + "' and CreateTime='" + dtime.ToString("yyyy-MM-dd HH:mm") + "'";
            DataTable dt = GetDataTable(query);
            string queryServer = "select mzjldid,RecordTime,NIBPS,NIBPD,NIBPM,RRC,HR,Pulse,SpO2,ETCO2,TEMP,CVP from Adims_mzjld_Point "
            ...
            {
                string copy = "insert into Adims_mzjld_Point(mzjldid,RecordTime,NIBPS,NIBPD,NIBPM,RRC,HR,Pulse,SpO2,ETCO2,CVP,TEMP) "
                    + "values('" + dt.Rows[0]["mzjldid"] + "','" + Convert.ToDateTime(dt.Rows[0]["CreateTime"].ToString()) + "','" + dt.Rows[0]["NIBPS"] + "','" + dt.Rows[0]["NIBPD"] + "',"
                    + "'" + dt.Rows[0]["NIBPM"] + "','" + dt.Rows[0]["RRC"] + "','" + dt.Rows[0]["HR"] + "','" + dt.Rows[0]["Pulse"] + "', '" + dt.Rows[0]["SpO2"] + "',"
                    + "'" + dt.Rows[0]["ETCO2"] + "','" + dt.Rows[0]["ARTS"] + "','" + dt.Rows[0]["TEMP"] + "')";
        ...
        /// GE  总数据
        /// </summary>
        public int insertJianCeDataMZJLD(int mzjldid, int nibps, int nibpd, int nibpm, int rrc, int hr, int pulse, int spo2, int etco2, double temp,int CVP, string now)
        {

            string insert = "insert into Adims_MonitorRecord(mzjldid,NIBPS,NIBPD,NIBPM,RRC,HR,Pulse,SpO2,ETCO2,TEMP,ARTS,CreateTime) values(" + mzjldid + "," + nibps + "," + nibpd + "," + nibpm + "," + rrc + "," + hr + "," + pulse + "," + spo2 + "," + etco2 + "," + temp + "," + CVP + ",'" + now + "')";
