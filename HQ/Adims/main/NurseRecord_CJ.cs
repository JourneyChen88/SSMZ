    ...
    public partial class NurseRecord_CJ : Form
    {
       ...
       private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {     
           ...
           e.Graphics.DrawString("床号: " + tbBedNO.Text.Trim(), zt9, Brushes.Black, x + 5, y);
            ...
            {            
                ...
                e.Graphics.DrawLine(pb1, new Point(xx, y), new Point(xx, y + 25 * rowCount));
                xx = xx + 50;
            }           
            y = y + 50; y1 = y + 25;
        ...
        private void button3_Click(object sender, EventArgs e)//绑定器械
        {
            ...
            dgvQXQD.Rows.Clear();
            dgvQXQD.Rows.Add(j + 1);
            for (int i = 0; i < dtCount; i++)
            {
                if (i % 3 == 0)
                {
                    dgvQXQD.Rows[i / 3].Cells[0].Value = dt.Rows[i][0];
                    dgvQXQD.Rows[i / 3].Cells[1].Value = dt.Rows[i][1];
                }
                else if (i % 3 == 1)
                {
                    dgvQXQD.Rows[i / 3].Cells[4].Value = dt.Rows[i][0];
                    dgvQXQD.Rows[i / 3].Cells[5].Value = dt.Rows[i][1];
                }
                else if (i % 3 == 2)
                {
                    dgvQXQD.Rows[i / 3].Cells[8].Value = dt.Rows[i][0];
                    dgvQXQD.Rows[i / 3].Cells[9].Value = dt.Rows[i][1];
                }
            }            
        }
