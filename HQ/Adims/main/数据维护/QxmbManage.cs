...
namespace main
{
    public partial class QxmbManage : Form
    {
        ...
        {
            DataTable dt = DAL.SelectqxmcInmodel(comboBox1.Text,cmbLB.Text);
            dataGridView1.DataSource = dt.DefaultView;
            //DataTable dts = DAL.SelectNum(comboBox1.Text.Trim(), cmbLB.Text);
            ...
            if (MessageBox.Show("确定修改", "提示", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                ...
                { 
                    ...
                    try
                    {
                        ...
                        flag = DAL.UpdateQxType(qxmbm,cmb);
                        //        j++;
                        //    }
                        //}
                    }
        ...
        adims_DAL.Dics.QxqdModelDal _QxqdModelDal = new QxqdModelDal();
        private void button2_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确定删除", "提示", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                ...
                int dts = _QxqdModelDal.DeleteqxqdModel(Convert.ToInt32(comboBox1.SelectedValue));
                int dt = _QxqdModelDal.DeleteqxqdType(comboBox1.Text);
                if (dt > 0)
                {
        ...
        private void tsmiAdd_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(Convert.ToString(cmbLB.Text.Trim())))
            {
                MessageBox.Show("请选择模板器械类别");
                this.textBox1.Focus();
            }
            else
            {
                int bz = 0;
                if (cmbLB.Text.Trim() == "敷料类")
                {
                    bz = 1;
                }
                else if (cmbLB.Text.Trim() == "物品类")
                {
                    bz = 2;
                }
                else if (cmbLB.Text.Trim() == "器械类")
                {
                    bz = 3;
                }
                int flag = 0;
                flag = DAL.SelectqxqdModel(Convert.ToInt32(comboBox1.SelectedValue), cmbLB.Text.Trim(), bz);
                if (flag > 0)
                {
                    DgvBind();
                }
