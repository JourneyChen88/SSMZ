...
namespace main.权限管理
{
    public partial class ZhichengManage : Form
    {
        ...
        {
            pos_set = _UserDal.GetPositionJurisdictionAll();
        ...
        {
            PositionJurisdictionModel pj = new PositionJurisdictionModel();
            pj.position = position_txt.Text; 
            _UserDal.AddPositionJurisdiction(pj);
            pos_set = _UserDal.GetPositionJurisdictionAll();
            dataGridView1.DataSource = pos_set.Tables[0];
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            else if (MessageBox.Show("您真的要删除吗？", "警告", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                _UserDal.DeletePositionJurisdiction(pj);
                _UserDal.DeleteUserByPosition(dataGridView1.Rows[i].Cells[1].Value.ToString());
                pos_set = _UserDal.GetPositionJurisdictionAll();
                dataGridView1.DataSource = pos_set.Tables[0];
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else 
            {
                ...
                pj.position = position_txt.Text;
                _UserDal.UpdatePositionJurisdiction(pj);
                _UserDal.UpdateUserPosition(dataGridView1.Rows[i].Cells[1].Value.ToString(), position_txt.Text);                
                button2.Text = "修改";
                flag = 0;
                pos_set = _UserDal.GetPositionJurisdictionAll();
                dataGridView1.DataSource = pos_set.Tables[0];
