...
namespace main.PACU_LEVEL
{
    public partial class PACUaddQt : Form
    {
        ...
        {
            sss.Clear();
            DataTable dtYDY = _YongyaoListDal.GetPACU_qt(mzjldid);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (textBox1.Text != "")
            {
                ...
                mz1.Sysj = dtStart;
                mz1.Bz = 1;
                int m = _YongyaoListDal.InsertPACU_qt(mzjldid, mz1);
                if (m > 0) 
                    BindQtList();
                else 
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count == 1)
            {
                ...
                {
                    int p = _YongyaoListDal.UpdatePACU_qt_EndTime(mzjldid, DateTime.Now, Convert.ToInt32(dataGridView1.SelectedRows[0].Cells[0].Value));
                    BindQtList();
                }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count == 1)
            {
                int q = _YongyaoListDal.DeletePACU_Qt(mzjldid, Convert.ToInt32(dataGridView1.SelectedRows[0].Cells[0].Value));
                BindQtList();
                if (q != 1)
