...
namespace adims_DAL
{
    public class AdimsProvider
    {
       ...
       // 更新麻醉记录单1
        ...
        {
            string sql = "select patname 姓名,patbedno 床号,patdpm 科室,patsex 性别,patage 年龄,odate 日期,case state when '1'  then  '手术中'  when  '0'  then '手术结束' end 状态 from Adims_OTypesetting as a inner join ssjstate as s on a.patid=s.patid where CONVERT(varchar, Odate , 23 )='" + dtime + "'";
            return dBConn.GetDataTable(sql);
        }
        /// <summary>
        /// 查询排班
