...
namespace main
{
    public partial class addszsj : Form
    {
        ...
        /// </summary>
        /// <param name="l"></param>
        public addszsj(List<adims_MODEL.szsj> l, int mzid, int type1)
        {
            type = type1;
            szsj = l;
            MZID = mzid;
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            if (textBox1.Text != "")
            {
                ...
                if (s.Name == "测血糖")
                {
                    i = acon.InsertIntoSZSJ(MZID, textBox1.Text + " " + textBox2.Text + "mmol/L", DateTime.Now, type);
                }
                else
                {
                    i = acon.InsertIntoSZSJ(MZID, textBox1.Text , DateTime.Now, type);
                }
                if (i > 0)
                {
                    szsj.Add(s);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedCells.Count == 1)
            {
                if (MessageBox.Show("确定删除?", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    int j = 0;
                    foreach (adims_MODEL.szsj mz in szsj)
                    {
                        if (mz.Name == dataGridView1.CurrentRow.Cells[2].Value.ToString())
                        {
                            int i = acon.deleteSZSJ(ID, type);
                            szsj.Remove(mz);
                            j++;
                        }
                        if (j > 0)
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (comboBox1.Text.Trim() != "其他类")
            {
                DataTable dt = PDAL.GetAdims_szsjListByType(comboBox1.Text.Trim());
