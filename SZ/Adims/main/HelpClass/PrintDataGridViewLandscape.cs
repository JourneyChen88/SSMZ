...
namespace main
{
    ...
    /// 打印DataGridView
    /// </summary>
    public static class PrintDataGridViewLandscape
    {
        ...
        public static void Print(DataGridView dataGridView, string title, string title2, short DaYinNum)
        {
            try
            {
                ...
                ppvw.PrintPreviewControl.Zoom = 1.2;
                PrintDocument printDoc = new PrintDocument();

                //A4纸
                printDoc.DefaultPageSettings.PaperSize = new PaperSize("16K", 740, 1020); //PaperSize("A4", 800, 1100);

                //设置横向打印
                printDoc.DefaultPageSettings.Landscape = true;
                //设置边距
                printDoc.DefaultPageSettings.Margins = new Margins(60, 50, 60, 60);
        ...
        private static void printDoc_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            Font textFont = new Font("宋体", 8);
            Font TopFont = new Font("黑体", 9);
            ...
            {
                //画第二标题
                e.Graphics.DrawString(titleName2, TopFont, Brushes.Black, x + 120, y);
                Ycount = y;

                //第二标题尺寸
                SizeF titleSize2 = e.Graphics.MeasureString(titleName2, textFont, e.MarginBounds.Width);
            ...
            foreach (DataGridViewColumn column in dgv.Columns)
            {
                ...
                //表头高度
                int temp = (int)e.Graphics.MeasureString(column.HeaderText, TopFont,
            ...
            foreach (DataGridViewColumn column in dgv.Columns)
            {
                ...
                //画内容
                e.Graphics.DrawString(column.HeaderText, TopFont, new SolidBrush
            ...
            while (rowIndex < dgv.Rows.Count)
            {
                ...
                e.Graphics.DrawString(aInt.ToString(), TopFont, Brushes.Black, x - 20, y + 10);
                aInt++;
                ...
                if (row.Visible)
                {
                    ...
                    foreach (DataGridViewCell cell in row.Cells)
                    {
                        ...
                        //行高
                        int temp = (int)e.Graphics.MeasureString(cell.Value.ToString(), textFont,
                    ...
                    foreach (DataGridViewCell cell in row.Cells)
                    {
                        ...
                        if (cell.Value != null)
                        {
                            ...
                            textFont, columnWidth).Width) / 2;

                            if (cenderWidth < 0) cenderWidth = 0;

                            //内容居中要加的高度
                            float cenderHeight = (rowHeight + padding - e.Graphics.MeasureString(cell.Value.ToString(),
                            textFont, columnWidth).Height) / 2;

                            if (cenderHeight < 0) cenderHeight = 0;


                            //画内容
                            e.Graphics.DrawString(cell.Value.ToString(), textFont, new SolidBrush
            ...
            //画页脚
            e.Graphics.DrawString(footer, TopFont, Brushes.Black, x + 580, Ycount);
            //e.Graphics.DrawString(footer, textFont, Brushes.Black, x + (pagerWidth - e.Graphics.MeasureString
            //(footer, textFont).Width) / 2, e.MarginBounds.Bottom);
            page++;
