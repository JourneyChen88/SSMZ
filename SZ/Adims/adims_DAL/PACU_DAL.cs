...
namespace adims_DAL
{
    public class PACU_DAL
    {
        ...
        public int updateYaoPin(string ybName,string ypName,int cxyy,int ID,string dw,string yl,string zrff) 
        {
            string sql = "update Adims_YaoPinBag set BagName='" + ybName + "', ypName='" + ypName + "', yl='" + yl + "', dw='" + dw + "', zrff='" + zrff + "', cxyy='" + cxyy + "' where id='" + ID + "'";
            return dBConn.ExecuteNonQuery(sql);
        }
        public DataTable GetYongYaoBag(string bagname) ///
        {
            //string sql = "SELECT ypcname,yl,dw,zrff,xyy FROM Adims_YaoPinBag where bagname='" + bagname + "' ";
        ...
        public DataTable GetPACU(int mzid) /// 获取PACU信息
        {
        ...
        {
            string sql = "INSERT INTO Adims_PACU_SZ(mzjldid,otime,savetime,jcsjjg) VALUES ('" + mzid + "','" + otime + "','" + vtime + "','5')";
            return dBConn.ExecuteNonQuery(sql);
        }
